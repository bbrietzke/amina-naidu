...
class TestLeagueManager(unittest.TestCase):
    def test_empty_games(self):
        with DatabaseService('/tmp/test_empty_games.sqlite3') as db:
    ...
    def test_add_game_for_week(self):
        ...
        with DatabaseService('/tmp/test_add_game_for_week.sqlite3') as db:
            ...
            with LeagueManager(db) as lm:
                ...
                self.assertGreater(lm.save_game(0, start_week = this_week, url = "https://example.org/game/2", title = "testing 2"), 1)
                self.assertGreater(lm.save_game(0, start_week = this_week + 1, url = "https://example.org/game/3", title = "testing 3"), 2)
